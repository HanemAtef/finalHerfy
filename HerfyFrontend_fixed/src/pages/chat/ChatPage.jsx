...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const joinRoom = () => {
      socket.emit("joinRoom", orderId, (result) => {
        if (!result?.ok)
          console.error("Unable to join chat room:", result?.error);
      });
    };
    ...
    socket.on("messageDeleted", ({ _id }) => {
      setMessages((prev) =>
        prev.map((m) =>
          m._id === _id ? { ...m, deleted: true, text: "", mediaUrl: null } : m,
        ),
      );
    });
  ...
  const isChatOpen =
    !currentOrder || CHAT_OPEN_STATUSES.includes(currentOrder.status);
  ...
  const sendMessage = (messageText) => {
    ...
    if (socket) {
      ...
      messageService
        .sendMessage({ orderId, text: trimmed, type: "text" })
        .then((res) => {
  ...
  const handleDeleteMessage = (msg) => {
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m._id === msg._id
          ? { ...m, deleted: true, text: "", mediaUrl: null }
          : m,
      ),
    );
  };
  ...
  const handleDeleteConversation = async () => {
    ...
    setMessages((prev) =>
      prev.map((m) => ({ ...m, deleted: true, text: "", mediaUrl: null })),
    );
    setMenuOpen(false);
  };
    ...
    <div className="flex h-screen flex-col bg-neutral">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        <div className="text-center">
          <span className="rounded-full bg-white px-4 py-1 text-xs text-textGray shadow-sm">
            اليوم
          </span>
        </div>
        ...
        {messages.map((msg) => {
          ...
          return (
            <div
              key={msg._id}
              className={`group flex ${own ? "justify-start" : "justify-end"}`}
            >
              <div
                className={`max-w-[75%] ${own ? "items-start" : "items-end"} flex flex-col`}
              >
                ...
                <span className="mt-1 text-xs text-textGray">

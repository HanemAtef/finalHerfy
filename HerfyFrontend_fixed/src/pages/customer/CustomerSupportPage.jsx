...
const QUICK_INQUIRIES = [
  'لدي استفسار بخصوص الطلبات',
  'أواجه مشكلة في الدفع',
...
export default function CustomerSupportPage() {
  ...
  if (loading) return <LoadingSpinner text="جاري فتح قناة الدعم الفني..." />;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-bold text-textDark text-sm">فريق إدارة ودعم حرفي</h2>
            ...
            <p className="text-[11px] text-textGray flex items-center gap-1 mt-0.5">
              ...
              <span>محادثة مباشرة مع مسؤولي المنصة</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-neutral/40">
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center p-6">
            ...
            <h3 className="text-base font-bold text-textDark">مرحباً بك في مركز المساعدة</h3>
            <p className="mt-1 text-xs text-textGray max-w-xs leading-relaxed">
              يسعدنا خدمتك والرد على استفساراتك حول الحجوزات، الفواتير، أو المشاكل التقنية.
            </p>
            ...
            <div className="mt-5 flex flex-wrap justify-center gap-2 max-w-md">
              {QUICK_INQUIRIES.map((q, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => setText(q)}
                  className="rounded-xl border border-primary/20 bg-white px-3 py-1.5 text-xs font-semibold text-primary shadow-sm hover:bg-primary/5 hover:border-primary transition-all"
                >
                  {q}
                </button>
              ))}
          ...
          messages.map((msg) => {
            const isMe = msg.senderRole === 'customer' || msg.senderId === user?._id;
            return (
              <div
                key={msg._id}
              ...
              >
                <div className="flex items-center gap-1.5 px-1 mb-1 text-[10px] text-textGray">
                  <span>{isMe ? 'أنت' : msg.senderName || 'إدارة حرفي'}</span>
                  <span>•</span>
      ...
      <form onSubmit={handleSend} className="border-t border-neutral bg-white p-3">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-white shadow-sm hover:bg-primary/90 transition-all disabled:opacity-50 active:scale-95"
          >

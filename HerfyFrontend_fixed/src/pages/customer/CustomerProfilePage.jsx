...
import { FaSignOutAlt, FaUser, FaCalendarAlt, FaHeart, FaCog, FaCamera, FaArrowRight, FaFlag } from 'react-icons/fa';
            ...
            to ? (
              ...
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-lg px-4 py-3 text-sm ${
                  active ? 'border-r-4 border-primary bg-primary/5 text-primary font-medium' : 'text-textGray'
                }`}
              >
                <Icon size={16} /> {label}
              </button>
      ...
      <form onSubmit={handleSave} className="flex-1">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-textDark">إعدادات الملف الشخصي</h1>
        ...
        {savedMsg && (
          <div className="mb-4 rounded-lg bg-tertiary/10 px-4 py-3 text-sm text-tertiary">{savedMsg}</div>
        )}
        {error && (
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="card lg:col-span-2">
            <div className="grid items-center gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-textGray">لا يمكن تغيير البريد الإلكتروني</p>
              </div>
            </div>
          </div>
          ...
          <div className="card bg-primary text-white lg:col-span-1">
            ...
            <p className="text-sm opacity-80 mb-4">
              لقد قمت بإنجاز {orders.filter((o) => o.status === 'completed').length} مهمة مع حرفينا.
            </p>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between border-t border-white/20 pt-3">
                <span>إجمالي الحجوزات</span><span className="font-bold">{orders.length}</span>
              </div>
              <div className="flex items-center justify-between border-t border-white/20 pt-3">
                <span>الحالة</span>
                <span className="flex items-center gap-2 font-bold">
                  <span className="h-2 w-2 rounded-full bg-tertiary" /> عميل نشط
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="card mt-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/customer/dashboard" className="text-sm text-primary">عرض الكل ←</Link>
          </div>
          {orders.slice(0, 2).map((order) => (
            <div key={order._id} className="flex items-center gap-4 border-b border-borderGray py-3 last:border-0">
              ...
              <span className="text-xs text-tertiary">{ORDER_STATUS_LABELS[order.status]}</span>
            </div>
          ))}
          {orders.length === 0 && (
            <p className="py-4 text-center text-sm text-textGray">لا توجد حجوزات بعد</p>
          )}
        </div>
      </form>
    </div>
